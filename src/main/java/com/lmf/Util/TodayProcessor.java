...
import com.lmf.entity.News;
import com.lmf.entity.Url;
...
public class TodayProcessor implements PageProcessor {
    ...
    private static String[] keys = KeyUtil.getKeys();
...
//    }
     ...
     * @return
     */
    public static List<News> getNewsList(List<Url> urls){
      /*  for(int index = 0; index < 12; index++){
            indexMap.put(keys[index], index);
        }
        List<Url> urls = urlsList;*/
//        String url = "https://www.toutiao.com/search_content/?offset="
//                + 0 + "&format=json&keyword=" + "镇江长江音乐节"
//                + "&autoload=true&count=20&cur_tab=1";
    ...
    public static void main(String[] args) {
        List<Url> list = getDetailUrls("https://www.toutiao.com/search_content/?offset=0&format=json&keyword" +
                "=NBA&autoload=true&count=20&cur_tab=1", "shijiuda");
        System.out.println("UrlLength"+list.size());
        List<News> nList = getNewsList(list);

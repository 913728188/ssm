...
public class CsdnBlogPageProcessor implements PageProcessor {
    ...
    public void process(Page page) {
        ...
        if (page.getUrl().regex("http://blog\\.csdn\\.net/" + username + "/article/details/\\d+").match()) {
            ...
            // 设置阅读人数
            csdnBlog.setView(0);//(Integer.parseInt(page.getHtml().xpath("//div[@class='article_r']/span[@class='link_view']")
                    //.regex("(\\d+)人阅读").get()));
            // 设置评论人数
            csdnBlog.setComments(0);
            /*(Integer.parseInt(page.getHtml()
                    .xpath("//div[@class='article_r']/span[@class='link_comments']").regex("\\((\\d+)\\)").get()));*/
            // 设置是否原创
            csdnBlog.setCopyright(0);//(page.getHtml().regex("bog_copyright").match() ? 1 : 0);
            // 把对象存入数据库
    ...
    public static void main(String[] args) {
        ...
        Spider.create(new CsdnBlogPageProcessor()).addUrl(url).thread(5).run();
